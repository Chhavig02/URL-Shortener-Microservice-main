...
async function initializeDatabase() {
  ...
  try {
    ...
    // In-memory fallback with duplicate alias checking
    const _inMemory = [];
    pool = {
      query: async (text, params) => {
        if (text.includes('SELECT count(*)')) return { rows: [{ count: _inMemory.length }] };
        if (text.includes('INSERT INTO urls')) {
          // Check for duplicate short_code before inserting
          const duplicateExists = _inMemory.some(item => item.short_code === params[1]);
          if (duplicateExists) {
            const err = new Error('duplicate key value violates unique constraint "urls_short_code_key"');
            err.code = '23505';
            throw err;
          }
